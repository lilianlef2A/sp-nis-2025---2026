...
|`history`|affiche l'hitorique des comande |`$ history`|
|`cd`|changer le dossier courant |`$ cd (chemin abs ou rel) `|
...
---------------
le chemin absolu vers un fichier ou un dossier est le chemain depuis la racine (*root*) `/` : par exemple un chemain relatife commence par `./`ou `../` : on part de 

la maison de l'utilisateur (`/homme/utilisateur` ou cd
)